  ...
  <div class="page-container">
    <el-card shadow="never">
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <el-select v-model="query.role" style="width: 130px" @change="handleSearch">
            <el-option label="全部角色" value="" />
            <el-option label="买家" value="BUYER" />
            <el-option label="商家" value="MERCHANT" />
            <el-option label="管理员" value="ADMIN" />
          </el-select>
          <el-select v-model="query.account_status" style="width: 130px" @change="handleSearch">
      ...
      <el-table :data="list" v-loading="loading">
        ...
        <el-table-column label="状态" width="90" align="center">
          <template #default="{ row }">
            <el-tag size="small" :type="statusTagType(row.account_status)">
        ...
        <el-table-column label="操作" width="140" fixed="right">
          <template #default="{ row }">
            <template v-if="row.role === 'ADMIN'">
              <span class="muted-text">受保护</span>
            </template>
            <template v-else-if="row.account_status === 'PENDING'">
              <span class="muted-text">待审核中</span>
            </template>
            <template v-else-if="row.account_status === 'ACTIVE'">
              <el-button size="small" type="danger" link @click="openBanDialog(row)">封禁</el-button>
            </template>
            <template v-else>
              <el-button size="small" type="success" link @click="handleUnban(row)">解封</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="banDialogVisible" title="封禁用户" width="480px">
      <div class="ban-user-name">
        封禁用户：<b>{{ currentUser?.username }}</b>（{{ currentUser?.email }}）
      </div>
...
const query = reactive({ page: 1, page_size: 10, keyword: '', role: '', account_status: '' })
...
function roleText(role) {
  return { BUYER: '买家', MERCHANT: '商家', ADMIN: '管理员' }[role] || role
}

function roleTagType(role) {
  return { BUYER: 'info', MERCHANT: 'warning', ADMIN: 'danger' }[role] || 'info'
}

function statusText(status) {
  return { ACTIVE: '正常', DISABLED: '封禁', PENDING: '待审核' }[status] || status
}

function statusTagType(status) {
  return { ACTIVE: 'success', DISABLED: 'danger', PENDING: 'warning' }[status] || 'info'
}
