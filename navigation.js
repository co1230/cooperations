...
function dashboard(user, index = 0) {
  ...
  const content = businessMode ? '<section class="panel" id="businessPage"><p role="status">正在加载…</p></section>' : index === 0 ? `<p class="hint">以下统计为演示数据，订单列表读取模拟接口。</p><section class="stats">${d.stats.map(s=>`<article class="stat"><span>${s[0]}</span><strong>${s[1]}</strong></article>`).join('')}</section>${table}` : isOrders ? table : isAdminPage ? '<section class="panel" id="adminPage"><p role="status">正在加载…</p></section>' : isProducts ? '<section class="panel" id="merchantProducts"><p role="status">正在加载商品…</p></section>' : plannedPageContent(title);
