...
const backend={MERCHANT:process.env.LUOBO_D_BACKEND||'http://127.0.0.1:8001',ADMIN:process.env.LUOBO_E_BACKEND||'http://127.0.0.1:8000'};
...
try {
  for(const member of members) {
    ...
    const server=await createViteServer({configFile:false,root,plugins:[bridge,member.role==='BUYER'?react():vue()],
      ...
      server:{host:'127.0.0.1',port:member.port,strictPort:true,open:false,
        ...
        proxy:member.role==='BUYER'?{}:{'/api':{target:backend[member.role],changeOrigin:true}},
  ...
  const server=http.createServer(async(req,res)=>{
    try {
      ...
      if(url.pathname==='/integration-config.json') {res.setHeader('Content-Type','application/json');res.end(JSON.stringify({targets,missingC:true,merchantBackendComplete:existsSync(path.join(repo,'directory/backend/config/db_conf.py'))}));return;}
  ...
  console.log('所有前端由 A 启动，成员源码不改写。D 后端使用8001，E后端使用8000。');
