...
function routeUser(user){try{user=accountService.currentUser();if(teamIntegration.route(user))return;user.role==='BUYER'?storeHome(user):dashboard(user)}catch(error){authView('login',escapeHtml(error.message))}}
